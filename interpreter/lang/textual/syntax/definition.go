...
var __SyntaxDefinition = [...] string {
    ...
    "decl_type = docs @type name! type_params impl type_def!",
      ...
      "type_def = native_type_def | record | interface | union | enum",
        ...
        "enum = @enum {! enum_item+,! }!",
          "enum_item = docs name",
    "decl_func = docs function variadic name! sig! body!",
    ...
    "decl_const = docs Const name! type! body!",
    "decl_method = docs @method receiver! .! name! type! body!",
      "receiver = name",
    "type = ref",
    ...
    "term = infix_term | lambda | if | when | each | block | ref_term " +
      ...
      "if = If (! cond+,! )! if_yes elif* Else! if_no",
        ...
        "elif = If (! cond+,! )! block!",
      "when = When (! expr! )! {! case+,! }!",
        "case = name+_bar pattern? =>! expr!",
      "each = Each (! type! )! {! case+,! }!",
      "block = { binding* expr! }!",
        "binding = binding_plain | binding_cps",
          "binding_plain = let pattern! =! expr! ,!",
            "let = Let | Const",
          "binding_cps = _at ref! cps_pattern expr! ,!",
            "cps_pattern? = pattern = ",
      "ref_term = new ref",
        "new? = New new_tag",
          "new_tag? = : name!",
